...
This document defines the writing conventions for FreeSign documentation.
...
1. **Users** - People using the FreeSign web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own FreeSign instance
...
```typescript
// Basic usage
const document = await freesign.documents.create({
  title: "Contract",
  file: pdfBuffer,
});

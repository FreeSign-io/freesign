# Docker Setup for FreeSign

The following guide will walk you through setting up FreeSign using Docker. You can choose between a production setup using Docker Compose or a standalone container.
...
This setup includes a PostgreSQL database and the FreeSign application. You will need to provide your own SMTP details via environment variables.

1. Download the Docker Compose file from the FreeSign repository: [compose.yml](https://raw.githubusercontent.com/ghcr.io/freesign-io/freesign/release/docker/production/compose.yml)
...
4. Set up your signing certificate. You have three options:
   ...
   **Option A: Generate Certificate Inside Container (Recommended)**
   ...
   docker exec -e CERT_PASS="$CERT_PASS" -it freesign-production-freesign-1 bash -c "
   ...
   # Restart container
   docker-compose restart freesign
   ```
...
This will start the PostgreSQL database and the FreeSign application containers.

6. Access the FreeSign application by visiting `http://localhost:3000` in your web browser.

## Option 2: Standalone Docker Container

If you prefer to host the FreeSign application on your container provider of choice, you can use the pre-built Docker image from DockerHub or GitHub's Package Registry. Note that you will need to provide your own database and SMTP host.

1. Pull the FreeSign Docker image:

```
docker pull ghcr.io/freesign-io/freesign
```
...
3. Access the FreeSign application by visiting the URL you provided in the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

## Success

You have now successfully set up FreeSign using Docker. You can start organizing and managing your documents efficiently.
...
```bash
# For Docker Compose
docker-compose logs -f freesign
...
Check the status of your FreeSign instance:
...
If you encounter any issues or have further questions, please refer to the official FreeSign documentation or seek assistance from the community.

## Advanced Configuration

The environment variables listed above are a subset of those that are available for configuring FreeSign. For a complete list of environment variables and their descriptions, refer to the table below:

Here's a markdown table documenting all the provided environment variables:

| Variable                                                       | Description                                                                                         |
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                                         | The port to run the FreeSign application on, defaults to `3000`.                                   |

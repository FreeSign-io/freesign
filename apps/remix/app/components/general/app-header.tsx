    ...
    >
      <div className="mx-auto flex w-full max-w-screen-xl items-center justify-between gap-x-4 px-4 md:justify-normal md:px-8">
        ...
        <div className="flex flex-row items-center space-x-4 md:hidden">
          ...
          <Link to="/inbox" className="relative flex h-11 w-11 items-center justify-center">
            <InboxIcon className="h-6 w-6 text-muted-foreground" />
            {unreadCountData && unreadCountData.count > 0 && (
              <span className="absolute right-0 top-0 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-semibold text-primary-foreground">
                {unreadCountData.count > 99 ? '99+' : unreadCountData.count}
              </span>
            )}
          </Link>

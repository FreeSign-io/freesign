---
title: Upgrades
description: Upgrade your self-hosted FreeSign installation to get the latest features, security patches, and bug fixes.
---
...
FreeSign follows [Semantic Versioning](https://semver.org/):
...
<Callout type="info">
  Review the [release notes](https://github.com/FreeSign-io/freesign/releases) before upgrading to
...
Check your running FreeSign version:

<Tabs items={['Docker', 'Docker Compose', 'Kubernetes']}>
<Tab value="Docker">

```bash
docker inspect freesign --format='{{.Config.Image}}'
```
...
```bash
kubectl get deployment freesign -n freesign -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
curl -s https://api.github.com/repos/freesign-io/freesign/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/FreeSign-io/freesign/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/freesign-io/freesign/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/FreeSign-io/packages/container/freesign/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker compose exec database pg_dump -U freesign freesign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl exec -n freesign deploy/postgres -- pg_dump -U freesign freesign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
pg_dump "postgresql://user:password@host:5432/freesign" > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl get secret freesign-secrets -n freesign -o yaml > secrets-backup.yaml
kubectl get configmap freesign-config -n freesign -o yaml > configmap-backup.yaml
```
...
```bash
docker pull ghcr.io/freesign-io/freesign:1.6.0
```
...
docker run -d \
  ...
  ghcr.io/freesign-io/freesign:1.6.0
```
...
services:
  freesign:
    image: ghcr.io/freesign-io/freesign:1.6.0
```
...
services:
  freesign:
    image: ghcr.io/freesign-io/freesign:${FREESIGN_VERSION:-latest}
```
...
```bash
kubectl set image deployment/freesign \
  freesign=ghcr.io/freesign-io/freesign:1.6.0 \
  -n freesign
```
...
```bash
kubectl rollout status deployment/freesign -n freesign
```

Watch pods transition:

```bash
kubectl get pods -n freesign -w
```
...
```bash
kubectl get pods -n freesign -o wide
```

Test the health endpoint:

```bash
kubectl port-forward svc/freesign 3000:80 -n freesign &
curl http://localhost:3000/api/health
```
...
FreeSign runs database migrations automatically when the container starts. No manual intervention is required.
...
```bash
docker logs freesign 2>&1 | grep -i migration
```
...
```bash
kubectl logs -l app.kubernetes.io/name=freesign -n freesign | grep -i migration
```
...
# Run migrations only
docker run --rm \
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/freesign" \
  ghcr.io/freesign-io/freesign:1.6.0 \
...
Read the [release notes](https://github.com/FreeSign-io/freesign/releases) carefully.
...
curl -O https://raw.githubusercontent.com/freesign-io/freesign/main/.env.example
...
kubectl rollout history deployment/freesign -n freesign

# Rollback to previous version
kubectl rollout undo deployment/freesign -n freesign

# Or rollback to a specific revision
kubectl rollout undo deployment/freesign -n freesign --to-revision=2
```
...
```bash
docker compose exec -T database psql -U freesign freesign < backup-20240115-120000.sql
```
...
```bash
psql "postgresql://user:password@host:5432/freesign" < backup-20240115-120000.sql
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start after upgrade">
    **Check logs:** `docker logs freesign`
  ...
  <Accordion title="Migration errors">
    ...
    **Test connectivity:**
    ...
    ```bash
    docker run --rm postgres:15 psql "postgresql://user:password@host:5432/freesign" -c "SELECT 1"
    ```
  ...
  <Accordion title="Application errors after upgrade">
    **If the application starts but behaves unexpectedly:**
    ...
    ```bash
    docker logs freesign 2>&1 | grep -i -E "(warn|error)"
    ```
  </Accordion>

---
title: Backups
description: Backup strategies for FreeSign data.
---
...
FreeSign stores data in three locations:
...
```bash
pg_dump -h localhost -U freesign -d freesign -F c -f freesign_backup.dump
```
...
```bash
docker compose exec database pg_dump -U freesign -F c freesign > freesign_backup.dump
```
...
```bash
pg_dump -h localhost -U freesign -d freesign -F c \
  -f "freesign_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U freesign -d freesign -F p -f freesign_backup.sql
```
...
```bash
pg_dump -h localhost -U freesign -d freesign | gzip > freesign_backup.sql.gz
```
...
```bash
cp /opt/documenso/cert.p12 /backup/freesign/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/freesign/cert.p12
```
...
```bash
aws s3 sync s3://your-freesign-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-freesign-bucket /backup/freesign/documents/
```
...
```bash
aws s3 sync s3://your-bucket /backup/freesign/documents/ \
...
Create a script to back up all FreeSign components:
...
S3_BUCKET="your-freesign-bucket"

# Create backup directory
mkdir -p "$BACKUP_DIR/$TIMESTAMP"

echo "Starting FreeSign backup: $TIMESTAMP"
...
tar -czf "$BACKUP_DIR/freesign_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/freesign_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/freesign_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "freesign_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "freesign_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: freesign_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# freesign-backup-docker.sh

set -e

BACKUP_DIR="/backup/freesign"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="freesign-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U freesign -F c freesign > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  freesign:/opt/documenso/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/freesign-backup.sh
```
...
0 2 * * * /usr/local/bin/freesign-backup.sh >> /var/log/freesign-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/freesign-backup-hourly.sh >> /var/log/freesign-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/freesign-backup.service`:

```ini
[Unit]
Description=FreeSign Backup

[Service]
Type=oneshot
ExecStart=/usr/local/bin/freesign-backup.sh
User=root
```

Create `/etc/systemd/system/freesign-backup.timer`:

```ini
[Unit]
Description=Daily FreeSign Backup
...
systemctl enable --now freesign-backup.timer
```
...
```bash
pg_restore -h localhost -U freesign -d freesign -c freesign_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U freesign -d freesign -c < freesign_backup.dump
```
...
```bash
psql -h localhost -U freesign -d freesign < freesign_backup.sql
```

For compressed backups:

```bash
gunzip -c freesign_backup.sql.gz | psql -h localhost -U freesign -d freesign
```
...
```bash
cp /backup/freesign/cert.p12 /opt/documenso/cert.p12
...
```bash
aws s3 sync /backup/freesign/documents/ s3://your-freesign-bucket
```
...
```bash
docker compose exec -T database pg_restore -U freesign -d freesign -c < backup.dump
```
...
```bash
# Check checksum
sha256sum -c freesign_backup.sha256

# Verify pg_dump format
pg_restore --list freesign_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp freesign_backup.tar.gz s3://your-backup-bucket/freesign/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 freesign_backup.tar.gz

# Decrypt
gpg --decrypt freesign_backup.tar.gz.gpg > freesign_backup.tar.gz
```

...
description: Diagnose and resolve common issues with your self-hosted FreeSign instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=freesign -n freesign -f
```
...
```bash
# Docker - find errors
docker logs freesign 2>&1 | grep -i error

# Docker - find warnings
docker logs freesign 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 freesign
```

### Health Checks

FreeSign exposes a health endpoint to verify the application is running:
...
```bash
docker exec freesign printenv | grep NEXT
```
...
```bash
docker compose exec freesign printenv | grep NEXT
```
...
```bash
docker inspect freesign --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats freesign

# Kubernetes
kubectl top pod -n freesign
```
...
<Steps>
  ...
  <Step>
    Change FreeSign's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "freesign"
```
...
```sql
ALTER USER freesign WITH PASSWORD 'newpassword';
```
...
```
Error: database "freesign" does not exist
```
...
```
Error: too many connections for role "freesign"
```
...
```
postgresql://user:password@host:5432/freesign?connection_limit=5
```
...
```bash
docker exec freesign printenv | grep SMTP
```
...
```bash
docker exec freesign nc -zv smtp.example.com 587
```
...
```bash
docker exec freesign printenv | grep SIGNING
```
...
```bash
docker exec freesign ls -la /opt/documenso/cert.p12
```
...
```
  ...
  <Step>
    Update the certificate file and restart FreeSign
  </Step>
</Steps>
...
```bash
docker exec freesign cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/FreeSign-io/freesign/discussions)**
- **[Discord](https://documen.so/discord)**: Join the FreeSign community Discord
- **[GitHub Issues](https://github.com/FreeSign-io/freesign/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@freesign.com directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs freesign` or `docker compose logs freesign` (or `kubectl logs` for

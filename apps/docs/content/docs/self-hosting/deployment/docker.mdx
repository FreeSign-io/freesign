---
title: Docker
description: Run FreeSign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for FreeSign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where FreeSign is accessible (e.g., `https://sign.example.com`) | -                             |
...
FreeSign requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/freesign" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="FreeSign" \
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/freesign
...
FreeSign provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' freesign
```

## Updating the Container

To update FreeSign to a new version:
...
The FreeSign container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs freesign`. Common causes: missing required environment

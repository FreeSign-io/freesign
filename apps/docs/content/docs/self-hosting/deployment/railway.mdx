---
title: Railway
description: Deploy FreeSign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/FreeSign-io/freesign/discussions) for help.
</Callout>
...
Once deployed, Railway provides a URL like `freesign-production.up.railway.app`.

Click the URL in the Railway dashboard to access your FreeSign instance.
...
If you prefer more control over the setup, deploy FreeSign manually:
...
- Fork or select the [FreeSign repository](https://github.com/FreeSign-io/freesign)
...
Click on the FreeSign service and go to the "Variables" tab. Add the following variables:
...
FreeSign runs database migrations automatically on startup.
...
- Click on your FreeSign service
...
Railway provides several options for scaling your FreeSign deployment:
...
<Steps>
<Step>
### Open your FreeSign service

In the Railway dashboard, select the FreeSign service.
...
In the Railway dashboard, select your FreeSign service and open "Settings".
...
<Callout type="warn">
  FreeSign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for FreeSign:
...
FreeSign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

FreeSign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your FreeSign service variables, set:
...
<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/FreeSign-io/freesign&branch=main&name=freesign-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img

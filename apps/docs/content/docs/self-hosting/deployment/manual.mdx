...
description: Deploy FreeSign on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `freesign` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/freesign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/freesign"
NEXT_PRIVATE_SMTP_FROM_NAME="FreeSign"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of FreeSign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run FreeSign as a background service on Linux:

```ini
# /etc/systemd/system/freesign.service
...
Type=simple
User=www-data
WorkingDirectory=/var/www/freesign
EnvironmentFile=/var/www/freesign/.env
...
```bash
sudo systemctl enable freesign
sudo systemctl start freesign
```

## Reverse Proxy with Nginx

A minimal Nginx configuration for proxying to FreeSign:

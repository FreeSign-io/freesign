...
description: Deploy FreeSign with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
```bash
mkdir freesign && cd freesign
curl -O https://raw.githubusercontent.com/freesign-io/freesign/release/docker/production/compose.yml
```
...
| `freesign` | Main application container, waits for database to be healthy |

The FreeSign container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and freesign services)
...
# Public URL where FreeSign is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://freesign:your-secure-database-password@database:5432/freesign
...
```
NAME                             STATUS              PORTS
freesign-production-database-1  running (healthy)   5432/tcp
freesign-production-freesign-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart freesign
```
...
For production, place FreeSign behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U freesign freesign > backup.sql

# Restore from backup
docker compose exec -T database psql -U freesign freesign < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/freesign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/freesign
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs freesign`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U freesign -d freesign -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs freesign | grep -i smtp`.
  </Accordion>

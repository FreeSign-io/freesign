---
title: Kubernetes
description: Deploy FreeSign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical FreeSign Kubernetes deployment consists of:
...
Create a dedicated namespace for FreeSign:
...
metadata:
  name: freesign
  labels:
    app.kubernetes.io/name: freesign
```
...
stringData:
  ...
  NEXT_PRIVATE_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/freesign'
  NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/freesign'
...
data:
  ...
  NEXT_PRIVATE_SMTP_FROM_NAME: 'FreeSign'
...
Create the FreeSign Deployment:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: freesign
          image: freesign-io/freesign:latest
      ...
      volumes:
        - name: signing-cert
          secret:
            secretName: freesign-signing-cert
            items:
              - key: cert.p12
                path: cert.p12
```
...
<Callout type="info">
  Pin to a specific image tag (e.g., `freesign-io/freesign:1.5.0`) in production instead of `latest`
...
```bash
kubectl create secret generic freesign-signing-cert \
  --namespace freesign \
...
type: Opaque
stringData:
  POSTGRES_USER: freesign
  POSTGRES_PASSWORD: your-secure-password
  POSTGRES_DB: freesign
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
Update your FreeSign secret to use the in-cluster database:

```yaml
NEXT_PRIVATE_DATABASE_URL: 'postgresql://freesign:your-secure-password@postgres.freesign.svc.cluster.local:5432/freesign'
NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://freesign:your-secure-password@postgres.freesign.svc.cluster.local:5432/freesign'
```

## Persistent Storage

FreeSign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
stringData:
  ...
  NEXT_PRIVATE_UPLOAD_BUCKET: 'your-freesign-bucket'
...
Scale FreeSign based on CPU and memory utilization:
...
<Accordions type="multiple">
  <Accordion title="Pods not starting">
    ...
    ```bash
    # Get pod status
    kubectl get pods -n freesign
    
    # Describe pod
    kubectl describe pod <pod-name> -n freesign
    ```
  ...
  <Accordion title="Database connection errors">
    ...
    ```bash
    kubectl run postgres-test --rm -it --image=postgres:15-alpine -n freesign -- psql "postgresql://user:password@host:5432/database" -c "SELECT 1"
    ```
  </Accordion>
  <Accordion title="Ingress not working">
    ...
    ```bash
    # Get Ingress status
    kubectl get ingress -n freesign
    
    # Describe Ingress
    kubectl describe ingress freesign -n freesign
    ```
  ...
  <Accordion title="Certificate errors">
    ...
    kubectl get secret freesign-signing-cert -n freesign -o yaml
    ```
    ...
    kubectl exec -it <pod-name> -n freesign -- ls -la /opt/documenso/
    ```
  </Accordion>

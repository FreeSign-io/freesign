...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your FreeSign deployment.
---
...
FreeSign requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://freesign:password@localhost:5432/freesign
```

**Remote server with SSL:**

```
postgresql://freesign:password@db.example.com:5432/freesign?sslmode=require
```
...
```
postgresql://freesign:p%40ss%23word@localhost:5432/freesign
```

## Environment Variables

FreeSign uses two database connection variables:
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@host:5432/freesign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@host:5432/freesign
```

### Automatic Detection

FreeSign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, FreeSign will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/freesign?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/freesign
```
...
FreeSign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/freesign?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/freesign?sslmode=require
```
...
```
postgresql://user:password@host:5432/freesign?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/freesign?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  freesign-io/freesign:latest
```

## Running Migrations

Database migrations update your schema when upgrading FreeSign.

### Automatic Migrations

When running FreeSign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/freesign?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/freesign?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/freesign?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://freesign:[password]@your-instance.region.rds.amazonaws.com:5432/freesign?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://freesign:[password]@your-instance.region.rds.amazonaws.com:5432/freesign?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://freesign:[password]@/freesign?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://freesign:[password]@[public-ip]:5432/freesign?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://freesign@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/freesign?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/freesign?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d freesign -F c -f freesign_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d freesign -c freesign_backup.dump
```
...
Key parameters for FreeSign workloads:
...
FreeSign includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER freesign WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Database does not exist">
    ...
    ```sql
    CREATE DATABASE freesign;
    GRANT ALL PRIVILEGES ON DATABASE freesign TO freesign;
    ```
  </Accordion>
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO freesign;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO freesign;
    ```
  </Accordion>
</Accordions>

...
S3 storage is recommended for production deployments. FreeSign supports AWS S3 and any S3-compatible storage service.
...
```bash
aws s3 mb s3://your-freesign-bucket --region us-east-1
```
...
[
  {
    ...
    "AllowedOrigins": ["https://your-freesign-domain.com"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
```bash
aws s3api put-bucket-cors --bucket your-freesign-bucket --cors-configuration file://cors.json
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::your-freesign-bucket/*"
    }
  ]
}
```
...
Using Docker Compose with FreeSign:
...
```bash
NEXT_PUBLIC_UPLOAD_TRANSPORT=s3
NEXT_PRIVATE_UPLOAD_BUCKET=freesign
...
FreeSign works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for FreeSign
...
FreeSign does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, FreeSign can read from both backends. The `DocumentData.type` field indicates where each document is stored.

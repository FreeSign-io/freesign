...
description: Complete reference for all environment variables used to configure a self-hosted FreeSign instance.
---

## Required Variables

These variables must be set for FreeSign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your FreeSign instance (e.g., `https://sign.example.com`)                   |
...
FreeSign requires PostgreSQL 14 or higher.
...
FreeSign supports multiple email transports for sending notifications.
...
FreeSign can store documents in the database or S3-compatible storage.
...
FreeSign requires a certificate to digitally sign documents.
...
FreeSign can use Google Vertex AI for recipient and field detection.
...
FreeSign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
| `NEXT_PRIVATE_REDIS_PREFIX`        | No       | Key prefix for Redis queues (useful when sharing an instance) | `freesign` |
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.freesign.com](https://license.freesign.com) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.
...
# Database
NEXT_PRIVATE_DATABASE_URL="postgres://user:password@localhost:5432/freesign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgres://user:password@localhost:5432/freesign"

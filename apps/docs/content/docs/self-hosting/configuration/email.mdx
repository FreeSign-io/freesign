...
FreeSign supports four email transport methods:
...
SMTP is the most common email transport. FreeSign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for FreeSign to use.
...
FreeSign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the FreeSign logo
...
Configure FreeSign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="FreeSign Test"
```
...
```bash
# Docker
docker logs freesign 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in FreeSign using the environment variables described above.

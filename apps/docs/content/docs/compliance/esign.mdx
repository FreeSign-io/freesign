...
## How FreeSign Supports Compliance

FreeSign provides features that support compliance with e-signature laws across jurisdictions:
...
FreeSign maintains an audit trail for each document, recording:
...
## What FreeSign Does NOT Provide

FreeSign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:

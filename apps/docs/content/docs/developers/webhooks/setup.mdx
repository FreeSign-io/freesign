...
Webhooks are HTTP callbacks triggered by specific events in FreeSign. When an event occurs (such as a document being signed), FreeSign sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in FreeSign, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
func main() {
	http.HandleFunc("/webhooks/freesign", webhookHandler)
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. FreeSign will retry failed deliveries.
</Callout>

## Configuring Webhooks in FreeSign via the Dashboard
...
FreeSign provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
FreeSign will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), FreeSign automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-FreeSign-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>

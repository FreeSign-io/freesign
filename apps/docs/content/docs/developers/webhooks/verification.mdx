...
Verifying webhook requests ensures that incoming payloads originate from FreeSign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How FreeSign Signs Webhooks

When you configure a webhook with a secret, FreeSign includes that secret in every webhook request via the `X-FreeSign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-FreeSign-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-FreeSign-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-FreeSign-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.FREESIGN_WEBHOOK_SECRET;
...
app.post('/webhooks/freesign', (req, res) => {
...
WEBHOOK_SECRET = os.environ.get('FREESIGN_WEBHOOK_SECRET')
...
@app.route('/webhooks/freesign', methods=['POST'])
def handle_webhook():
    received_secret = request.headers.get('X-FreeSign-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in FreeSign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in FreeSign             |
...
3. Update the webhook configuration in FreeSign
...
app.post('/webhooks/freesign', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```

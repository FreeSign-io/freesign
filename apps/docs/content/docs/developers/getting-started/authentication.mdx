...
- A FreeSign account on any plan (Free, Individual, Team, or Enterprise)
...
- Log in to your FreeSign account
...
```bash
curl https://app.freesign.com/api/v2/documents \
...
```typescript
const response = await fetch('https://app.freesign.com/api/v2/documents', {
  method: 'GET',
  headers: {
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.freesign.com/api/v1/openapi).
</Callout>
...
FREESIGN_API_KEY=api_xxxxxxxxxxxxxxxx
```

```typescript
const client = new FreeSign({
  apiKey: process.env.FREESIGN_API_KEY,
});
```

...
Register your endpoint in the FreeSign dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/freesign</code>)
  </Step>
  <Step>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>FREESIGN_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class FreeSignApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof FreeSignApiError) {
...
- [API Reference](https://openapi.freesign.com/) - Full endpoint documentation
